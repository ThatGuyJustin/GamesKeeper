...
from GamesKeeper import NO_EMOJI_ID, YES_EMOJI_ID, NO_EMOJI, YES_EMOJI, bot_config, Emitter
...
class DevPlugin(Plugin):
    ...
    def server_give_admin(self, event, server):
        ...
        if server == 1:
            try:
                server_one.get_member(event.author.id).add_role(
                    bot_config.emoji_servers['admin_roles']['server_one'])
            ...
            try:
                server_two.get_member(event.author.id).add_role(
                    bot_config.emoji_servers['admin_roles']['server_two'])
                return event.msg.add_reaction(YES_EMOJI)
            except:
